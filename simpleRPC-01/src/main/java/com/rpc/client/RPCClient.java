...
/**
 * RPC客户端：调用服务器端的方法
 * 客户端建立socket连接，标定主机ip地址，指定程序使出的端口号，
 * 将socket作为输入输出流的target来创建输入输出流对象
 * 客户端通过输出流传id给服务器，刷新流
 * 客户端通过输入流获取服务器的返回对象，打印；
 *
 * host主机名：用于回送地址。主机名对应的ip地址，可以和别人通信（一个主机是一栋楼，这栋楼的名字是这个主机的Ip地址）
 * port端口号：一栋楼有很多个房间可以使用，这就是端口。一个程序就是一个人，如果要跟另外一个主机通信，需要开一个房间给他的程序使用
 ...
 *  此对象引用的对象被传递地读，这样对象的完全等价的图形是由readObject重建
 */
