...
/**
 * RPC服务器端
 * 服务器端：创建ServerSocket对象监听客户端的连接（BIO），监听到连接之后，开启一个线程来处理，socket对象的回获取输入
 * 输出流作为target，初始化输入输出流，读取从客户端传过来地id，用输入流的readInt()读取出来
 * 调用getUserByUserId,给User赋值之后返回User对象，把Client想要的User对象通过输出流返回给客户端Client，
 * 输出流刷新（flush)
 *
 * ServerSocket:用户服务器端，监听客户端连接
 ...
 *  java.io.ObjectOutputStream.writeObject(Object obj): 此方法将指定的对象写入ObjectOutputStream,将对象的类，类的前命，
        ...
        try{
            ...
            while (true){
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try{
                            //socket对象的获取输入输出流作为target，初始化输入输出流
                            ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
                            ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());

                            //读取从客户端传过来的id，用readInt读取出来，调用getUserByUserId,给User赋值之后返回User对象
                            Integer id = ois.readInt();
                            User userByUserId =userService.getUserByUserId(id);//这个是Client想要Server调用的方法
