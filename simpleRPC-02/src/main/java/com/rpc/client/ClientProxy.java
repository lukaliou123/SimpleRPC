 ...
 * Java动态代理机制中有两个重要的类和接口InvocationHandler（接口）和proxy（类），实现动态代理的核心
 *
 * InvocationHandler接口：是Proxy代理实例的调用处理程序实现的一个接口，每一个proxy代理实例都有一个关联的调用处理程序，
 *      在代理实例调用方法时，方法调用被编码分派到调用处理程序的invoke方法。
 *      每一个动态代理类的调用处理程序都必须实现InvocationHandler接口类的Invoke方法来调用
 *
 * Proxy:该类用于动态生成代理类，只需传入目标接口、目标接口的类加载器以及InvocationHandler便可为目标接口生成代理类以及代理对象
...
public class ClientProxy implements InvocationHandler {
    ...
    // 初始化InterfaceName.初始化methodName，初始化Params,初始化ParamTypes
    ...
    /**
     * 动态代理，每一次代理对象调用方法，会经过此方法增强（反射获取request 对象，socket发送者客户端
     *
     */
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        //调用IOClient,通过输入流输出流进行request的数据传输，并返回服务器端传来的response
